...
CAMLprim value
Wrapper_FT_Get_Sfnt_Name_Count(value f)
{
    CAMLparam1(f);
    FT_Face face;
    ...
    CAMLreturn(Val_int(FT_Get_Sfnt_Name_Count(face)));
}

CAMLprim value
Wrapper_FT_Get_Sfnt_Name(value f, value i)
{
    CAMLparam2(f, i);
    CAMLlocal2(block, s);
    FT_Face face;
    ...
    error = FT_Get_Sfnt_Name(face, Int_val(i), &aname);
    if (error != 0)
        failwith("FT_Get_Sfnt_Name");

    s = caml_alloc_string(aname.string_len);
    memcpy(String_val(s), aname.string, aname.string_len);
    block = alloc_tuple(5);
    Store_field(block, 0, Val_int(aname.platform_id));
    Store_field(block, 1, Val_int(aname.encoding_id));
    Store_field(block, 2, Val_int(aname.language_id));
    Store_field(block, 3, Val_int(aname.name_id));
    Store_field(block, 4, s);
